    ...
    OFF_SET {
        ...
        public CursoredPage<CommunicationEntity> cursor(SelectQuery query, PageRequest pageRequest, DatabaseManager template) {
            ...
            if (entities.isEmpty()) {
                ...
                var firstCursor = getCursor(query.sorts(), entities.get(0));
                var nextCursor = getCursor(query.sorts(), entities.get(entities.size() - 1));
        ...
        public CursoredPage<CommunicationEntity> cursor(SelectQuery query, PageRequest pageRequest, DatabaseManager template) {
            ...
            if (entities.isEmpty()) {
                ...
                var beforeCursor = getCursor(query.sorts(), entities.get(0));
                var nextCursor = getCursor(query.sorts(), entities.get(entities.size() - 1));
