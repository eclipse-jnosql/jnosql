...
- Add Time Series mapping support through the new `jnosql-mapping-timeseries` module.
- Add `TimeSeriesTemplate` for mapping and querying time-oriented data.
- Add Jakarta Data repository integration for Time Series databases.
- Add CDI and configuration support for Time Series persistence.

=== Changed

- Migrate service discovery consumers to the `ServiceDiscovery` mechanism.
- Update the Jakarta NoSQL API dependency.

== 1.1.17  - 2026-08-29

=== Added

* Added support for asynchronous repository methods using `CompletionStage`.
* Added support for opt-in mapping settings providers.

=== Changed

* Improved service discovery across different class-loading environments.
* Improved class loader fallback behavior for service discovery.

=== Fixed

* Fixed `BigInteger` reading to preserve full numeric precision.
* Fixed pagination so total result counts are honored correctly.
* Fixed backward cursor navigation.
* Fixed query method parsing so `And` expressions use the expected precedence.

=== Maintenance

* Updated project licensing configuration.
* Updated the project version to `1.1.17`.
...
* Fixes Serialization bug with enum
* Avoid a constructor error when the constructor does not match the arguments

== [1.1.12] - 2026-01-05

=== Fixed

* avoid NPE on enum mapping
